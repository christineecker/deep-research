...
| `asset` | object \| null | yes | `{path, sha256, bytes, refmgr_paper_id, refmgr_attachment_id}` for a snapshot derived from a stored file, else `null`. Exactly one of two shapes: **wiki-relative** (`path` set, `assets/papers/pmid-12345678.pdf`, an explicit exception to S7 because the PDF library is shared across runs and PDFs are never duplicated per run — R13; `refmgr_paper_id`/`refmgr_attachment_id` both `null`), or **refmgr-backed** (`path` `null`; `refmgr_paper_id`/`refmgr_attachment_id` both non-empty strings naming the exact `data/refmgr/library.sqlite3` paper/attachment this snapshot was extracted from — plan Phase 5: "connect extractions to exact attachment/snapshot versions"). `sha256` is lowercase hex of the file's bytes (no `sha256:` prefix, matching `corpus.fulltext.sha256`). `bytes` is the file size as an integer. |
